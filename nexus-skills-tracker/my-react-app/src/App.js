  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", gap: "24px" }}>
        <Link to="/" style={{ color: "#4a4a6a", textDecoration: "none", fontSize: "12px", letterSpacing: "1px" }}>HOME</Link>
        <Link to="/dashboard" style={{ color: "#4a4a6a", textDecoration: "none", fontSize: "12px", letterSpacing: "1px" }}>DASHBOARD</Link>
        <Link to="/skills" style={{ color: "#4a4a6a", textDecoration: "none", fontSize: "12px", letterSpacing: "1px" }}>SKILLS</Link>
        <Link to="/profile" style={{ color: "#4a4a6a", textDecoration: "none", fontSize: "12px", letterSpacing: "1px" }}>PROFILE</Link>
      </div>

      <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
        <span style={{ color: "#4ade80", fontSize: "12px" }}>{user?.name}</span>
...
function HomePage({ user }) {
  return (
    <div style={{ ...pageStyle, padding: "60px 40px" }}>
      <h1 style={{ color: "#4ade80", fontSize: "48px", letterSpacing: "4px" }}>
        Welcome back, {user.name}! 🚀
      </h1>
      <p style={{ color: "#4a4a6a", fontSize: "16px", marginBottom: "40px" }}>
        You are logged in as <strong style={{ color: "#60a5fa" }}>{user.role}</strong>
      </p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "20px" }}>
        {[
          { title: "Skills", desc: "Track your learning progress", link: "/skills", color: "#4ade80" },
          { title: "Dashboard", desc: "View your stats and metrics", link: "/dashboard", color: "#60a5fa" },
          { title: "Profile", desc: "Manage your account settings", link: "/profile", color: "#f59e0b" },
        ].map(card => (
          <Link key={card.title} to={card.link} style={{ textDecoration: "none" }}>
            <div style={{
              background: "#0d0d14", border: "1px solid #1f1f2e",
              borderLeft: `3px solid ${card.color}`, padding: "24px", cursor: "pointer",
            }}>
              <h2 style={{ color: card.color, margin: "0 0 8px 0" }}>{card.title}</h2>
              <p style={{ color: "#4a4a6a", margin: 0, fontSize: "13px" }}>{card.desc}</p>
            </div>
          </Link>
        ))}
      </div>
...
function DashboardPage() {
  const stats = [
    ...
    { label: "Lessons Done", value: "14", color: "#a78bfa" },
  ];

  return (
    <div style={{ ...pageStyle, padding: "40px" }}>
      <h1 style={{ color: "#4ade80" }}>📊 Dashboard</h1>
      <p style={{ color: "#4a4a6a" }}>Your learning stats at a glance</p>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "16px", marginTop: "32px" }}>
        {stats.map(s => (
          <div key={s.label} style={{
            background: "#0d0d14", border: "1px solid #1f1f2e",
            borderLeft: `3px solid ${s.color}`, padding: "24px"
          }}>
            <div style={{ color: "#4a4a6a", fontSize: "11px", letterSpacing: "2px", marginBottom: "8px" }}>
              {s.label.toUpperCase()}
            </div>
            <div style={{ color: "#fff", fontSize: "36px", fontWeight: "bold" }}>{s.value}</div>
          </div>
        ))}
      </div>

      <div style={{ marginTop: "32px", background: "#0d0d14", border: "1px solid #1f1f2e", padding: "24px" }}>
        <h2 style={{ color: "#60a5fa", marginTop: 0 }}>🗺️ Your Roadmap</h2>
        {[
      ...
      <div style={{ maxWidth: "500px" }}>
        ...
        {skills.map(skill => (
          <div key={skill.id} style={{
            ...
            border: "1px solid #1f1f2e", borderLeft: "3px solid #4ade80",
...
function ProfilePage({ user }) {
  return (
    <div style={{ ...pageStyle, padding: "40px" }}>
      ...
      <div style={{ maxWidth: "400px", background: "#0d0d14", border: "1px solid #1f1f2e", padding: "28px" }}>
        {[
          ...
          <div key={f.label} style={{ marginBottom: "16px", paddingBottom: "16px", borderBottom: "1px solid #0f0f1a" }}>
            <div style={{ color: "#4a4a6a", fontSize: "10px", letterSpacing: "2px", marginBottom: "4px" }}>
              {f.label.toUpperCase()}
            </div>
            <div style={{ color: "#fff", fontSize: "14px" }}>{f.value}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h1 style={{ color: "#4ade80", textAlign: "center", letterSpacing: "6px", marginBottom: "4px" }}>NEXUS</h1>
        <p style={{ color: "#4a4a6a", textAlign: "center", fontSize: "11px", letterSpacing: "2px", marginBottom: "28px" }}>
          CORE SAAS PLATFORM
        </p>

        <div style={{ display: "flex", marginBottom: "24px", borderBottom: "1px solid #1f1f2e" }}>
  ...
  return (
    <BrowserRouter>
      ...
      <Routes>
        ...
        <Route path="/skills" element={user ? <SkillsPage /> : <Navigate to="/auth" />} />
